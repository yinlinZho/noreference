...
def duplication(filename): #video_name, image_file_paths
    logger = logging.getLogger('duplication')
    start_time = time.time()
    try:
        logger.info('duplication %s ' % (video_name, ))
        file = open(filename,'r+')
        newlines = list(set(file.readlines()))
        file.seek(0)
        file.truncate()
...
def sseq(image_file_rootpath,csvpath):
    '''
    ����
    '''
    ...
    print image_file_rootpath
    filename = matlab.output(image_file_rootpath,csvpath)

    #matlab.output(image_file_rootpath,'F:\\')
    return image_file_rootpath
...
def quality(video_name, image_file_paths, csv_basedir):
    ...
    try:
        ...
        score_list = sseq(image_file_paths)
        score_list
...
def video_quality(video_file_path, image_basedir, csv_basedir):
    ...
    try:
        ...
        filename = sseq(image_dir,csvpath)
        # ȥ��
